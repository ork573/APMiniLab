...
public class CircleQueue
{
    ...
    public void selectionSort() {	
        ...
        while (node1 != null) {	
            ...
            //key to be inserted into sorted slot
            LinkedList key = new LinkedList(node1);
            LinkedList keyslot = node1;

            //walks slots forwards marking the smallest value.
            while ( node2 != null) {
                String keyText = node2.getObject().toString();
                if (key.getObject().toString().compareTo(keyText) > 0 ) {
                    key = new LinkedList(node2);
                    keyslot = node2;
                }
                //advance index
                node2 = node2.getNext();
            ...
            //place key in insertion position
            keyslot.setObject(node1.getObject());
            node1.setObject(key.getObject());

            //advance insertion sort indexes
            node1 = node1.getNext();
            //return node2 to node1
            node2 = node1;
        } 
    }
